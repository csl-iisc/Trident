 ...
 */
static unsigned long isolate_src_freepages_block(struct compact_control *cc,
    ...
    bool strict)
{
  ...
  for (; blockpfn < end_pfn; blockpfn++, cursor++) {
     ...
     */
    if (cc->order != 18 && (!(blockpfn % SWAP_CLUSTER_MAX)
...
isolate_success:
    ...
    if (cc->order != 18 && cc->nr_migratepages == COMPACT_CLUSTER_MAX) {
      ++low_pfn;
      break;
    }
...
/* Params : zone and a bool largest.
 * If largest is true, this fucntion returns the pfn of the pageblock with
 * the maximum number of free pages (or) least number of pages to migrate.
 * However, if largest is false, this function will return the pfn of pageblock
 * with minimum number of free pages (or) maximum pages to migrate.
 */
static unsigned long pfn_pageblock_freepages(struct zone *zone, bool largest){
  int iterator = 0, mini_index = -1, maxi_index = -1;
  unsigned long start_pfn = zone->zone_start_pfn, zonesize = zone->spanned_pages;
  int num_zone_pageblocks = 0;
  unsigned long mini = 0, maxi = 0;
  struct page *page;
  mini = pageblock_nr_pages + 1;
  maxi = 0;

  zonesize += start_pfn & (pageblock_nr_pages-1);
  num_zone_pageblocks = roundup(zonesize, pageblock_nr_pages);
  num_zone_pageblocks = num_zone_pageblocks >> pageblock_order;

  for(iterator = 0; iterator < num_zone_pageblocks; iterator++){
    page = pfn_to_page(start_pfn + iterator * pageblock_nr_pages);
 ...
 * Based on information in the current compact_control, find blocks
 * suitable for isolating free pages from and then isolate them.
 * Only for order 18 pages.
 */
static void isolate_freepages_pud(struct compact_control *cc)
{
  struct zone *zone = cc->zone;
  struct page *page;
  unsigned long block_start_pfn;	/* start of current pageblock */
  ...
  /* We select pageblock with least number of free pages or highest number of
   * movable pages and migrate all the pages in migrate_pages to this block. */
 ...
 * Only for order 18 pages.
 */
static isolate_migrate_t isolate_migratepages_pud(struct zone *zone,
    struct compact_control *cc)
{
  unsigned long block_start_pfn;
  unsigned long block_end_pfn;
  unsigned long low_pfn, low_pfn_free, free_isolated = 0;
  struct page *page;
  const isolate_mode_t isolate_mode =
    (sysctl_compact_unevictable_allowed ? ISOLATE_UNEVICTABLE : 0) |
    (cc->mode != MIGRATE_SYNC ? ISOLATE_ASYNC_MIGRATE : 0);
   ...
   * need to schedule, or even abort async compaction.
   */
  if (cc->order != 18 && (!(low_pfn % (SWAP_CLUSTER_MAX * pageblock_nr_pages))
        && compact_should_abort(cc)))
    return ISOLATE_ABORT;
  ...
  if(cc->order == 18 && cc->nr_migratepages){
    free_isolated = isolate_src_freepages_block(cc, &low_pfn_free, block_end_pfn,
        &cc->src_freepages, false);
    map_pages(&cc->src_freepages);
  }
    ...
    struct compact_control *cc)
{
  ...
  if (cc->order != 18 && compact_scanners_met(cc)) {
    reset_cached_positions(zone);
